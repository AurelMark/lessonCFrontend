...
import { getExamsBySlug, updateExams } from "@/api/exams";

type UploadExamsImageArgs = { file: File; slug: string };

export default function AdminExamsEdit() {
  const t = useTranslations();
  const locale = useLocale() as LANG_TYPE;
  const { slug } = useParams() as { slug: string };

  const { data } = useQuery({
    queryKey: ["examsById", slug],
  ...
  useEffect(() => {
    if (useUserStoreState?.id && data) {
      (async () => {
        form.reset({
          id: data.id || "",
          createdBy: useUserStoreState.id,
          title: data.title,
          description: data.description,
          content: data.content,
          imageUrl: data.imageUrl,
          slug: data.slug,
          responsible: Array.isArray(data.groups)
            ? data.responsible.map((e) => e.id)
            : [],
          lessons: Array.isArray(data.lessons)
            ? data.lessons.map((e) => e.id)
            : [],
          groups: Array.isArray(data.groups)
            ? data.groups.map((e) => e.id)
            : [],
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const updateExamsMutation = useMutation<any, Error, ExamsForm>({
    mutationFn: (data) => {
      if (!data.id) {
        return Promise.reject(new Error("Exam id is required for update"));
      }
      return updateExams(data.id, data);
    },
    onSuccess: () => {
      toast.success(t("dashboard.adminExams.ExamsCreatedSuccess"));
      window.location.href = `/${locale}/dashboard/exams`;
    },
    onError: () => {
    ...
    <ContentLayout title="Exams">
      <div className="mb-4 bg-background px-4 py-8">
        ...
        <AdminExamsImage
          ...
          isPending={updateExamsMutation.isPending}
        ...
        <div className="mt-8 flex justify-center">
          ...
          >
            {updateExamsMutation.isPending ? (
              ...
              t("dashboard.adminExams.updateExam")
            )}
          </Button>
        </div>
      </div>
