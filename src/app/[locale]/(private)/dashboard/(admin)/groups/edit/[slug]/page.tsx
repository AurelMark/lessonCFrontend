...
import { getGroupById, updateGroups } from "@/api/groups";
import {
  getDictionaryExams,
...
export default function AdminGroupsEditPage() {
  ...
  const { data } = useQuery({
    queryKey: ["groupById", slug],
    queryFn: () => getGroupById(slug),
    staleTime: 0,
    gcTime: 0,
  });
  ...
  useEffect(() => {
    if (userState?.id && data) {
      form.reset({
        createdBy: data.createdBy.id,
        exams: Array.isArray(data.exams) ? data.exams.map((e) => e.id) : [],
  ...
  const editCourseMutation = useMutation<any, Error, GroupsForm>({
    mutationFn: (data) => updateGroups(data, slug),
    onSuccess: () => {
        ...
        <div className="mt-8 flex justify-center">
          ...
          >
            {editCourseMutation.isPending ? (
              ...
              t("dashboard.adminGroups.editGroup")
            )}
          </Button>
        </div>
      </div>
