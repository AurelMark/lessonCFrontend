...
import { LoginOTPPayload, loginOTPRequest } from "@/api/auth";
...
export function LoginOTPForm({
  ...
  const handleAuthToSystem = useMutation({
    mutationFn: (payload: LoginOTPPayload) => loginOTPRequest(payload),
    onSuccess: () => {
      toast.success(
        <>
          <div className="font-bold">{t("OTP.loginSuccess")}</div>
          <div className="text-muted-foreground">{t("OTP.redirectInfo")}</div>
          <div className="font-bold mt-2">{t("OTP.checkEmailTitle")}</div>
          <div className="text-muted-foreground">{t("OTP.checkEmailInfo")}</div>
        </>
      );
      setTimeout(() => {
        router.push(
          `/${locale}/login/otp/code?email=${encodeURIComponent(
